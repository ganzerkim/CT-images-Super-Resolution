...
net_input = Conv2D(filters=32, kernel_size=1, strides=1, padding='same', activation='relu')(inputs)

net1 = MG_dense(net_input, num_filters = 32, activation = True)
r1 = Add()([net_input, net1])
g1 = Conv2D(filters=32, kernel_size=1, strides=1, padding='same', activation='relu')(r1)

net2 = MG_dense(g1, num_filters = 32, activation = True)
r2 = Add()([r1, net2])
g2 = Conv2D(filters=32, kernel_size=1, strides=1, padding='same', activation='relu')(r2)

net3 = MG_dense(g2, num_filters = 32, activation = True)
r3 = Add()([r2, net3])
g3 = Conv2D(filters=32, kernel_size=1, strides=1, padding='same', activation='relu')(r3)

input_2 = Conv2D(filters=32, kernel_size=1, strides=1, padding='same', activation='relu')(inputs)
r4 = Add()([r3, input_2])

net = Conv2D(filters=64, kernel_size=3, strides=1, padding='same', activation='relu')(r4)
net = Conv2D(filters=75, kernel_size=3, strides=1, padding='same', activation='relu')(net)
net = Subpixel(filters=75, kernel_size=3, r=int(L*upscale_factor), padding='same')(net)
net = Conv2D(filters=32, kernel_size=3, strides=1, padding='same', activation='relu')(net)
net = Conv2D(filters=3, kernel_size=3, strides=2, padding='same', activation='relu')(net)
...
plot_model(model, to_file = 'Dense_Res_modified_200121.png')
...
history = model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=100, batch_size=16, callbacks=[
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=10, verbose=0, mode='auto', min_lr=1e-05)
])

model.save('D:\\H&E_dataset\\training_DenseResnet_CT_modified_200121.h5')
print("Saved model to disk")

#Evaluation
model.save_weights('D:\\H&E_dataset\\weight_training_DenseResnet_CT_modified_200121.h5')


load_model('C:\\Users\\MG\\Desktop\\H&E New dataset\\SuperResolutionData\\CT_imageset\\20200123\\')

model.load_weights('C:\\Users\\MG\\Desktop\\H&E New dataset\\SuperResolutionData\\CT_imageset\\20200123\\weight_training_DenseResnet_CT_modified_200121.h5')
...
cv.imwrite(test_img_path + '\\prd_200121.png', preds_re)
